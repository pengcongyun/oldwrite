...
$sql='select sob.organization_brand_name,c.alias,op.created,(case o.order_status when 1 then "下单成功" when 2 then "仓库确认" when 3 then "分拣完成" when 4 then "开始配送" else "配送完成" end),op.product_name,concat(opd.product_capacity,(case when opd.product_capacity_unit=2 then "升" else "毫升" end)) as rj,op.number_per_box,op.default_price,op.settlement_price,op.product_number,op.default_price*op.product_number as dd_total,op.settlement_price*op.product_number as js_total from order_product op join order_product_description opd on op.order_product_id=opd.order_product_id join `order` o on op.order_id=o.order_id join shop c on o.shop_id=c.shop_id join shop_organization_brand sob on sob.shop_organization_brand_id=c.shop_organization_brand_id where op.order_product_id>1 and o.order_status!=6 order by o.shop_id desc,op.created desc,o.order_status asc limit 3';
$stmt=mysqli_query($conn,$sql);
// PHP文件句柄，php://output 表示直接输出到浏览器
$fp = fopen('php://output', 'a');
// 输出Excel列头信息
$head = array('商铺品牌', '店铺名称', '订购日期', '订单状态', '商品名称', '商品规格', '计量/件', '商品单价', '结算单价', '订购数量', '订购金额', '结算金额');
foreach ($head as $i => $v) {
...
//http://www.old.com/sql/phpexcel.php?start=2018-11-02 14:00:00&end=2018-11-05 14:00:00