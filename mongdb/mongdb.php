...
//查看mongdb的数据库
//var_dump($conn->listDBs());exit;
// 选择comedy数据库，如果以前没该数据库会自动创建，也可以用$m->selectDB("comedy");
$db = $conn->test;
// 制定结果集（表名：user）
$collection = $db->user;
//新增，如果存在就更新
//$user = $collection->findOne(array('name' => "张三","email"=>'admin@three.com'));
$user = $collection->findOne(array('name' => "张三"));
if(empty($user)){
    $user = array('name' => "张三", 'email' => 'admin@three.com');
    $collection->insert($user);
}else{
    // 修改更新
    $newdata = array('$set' => array("email" => "tes@test.com"));
