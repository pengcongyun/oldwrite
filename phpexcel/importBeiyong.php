...
$count=0;
$objPHPExcel = $objReader->load('./resource/1111.xls'); //$filename可以是上传的表格，或者是指定的表格
...
for($shop_id=216;$shop_id<=234;$shop_id++){
    for($i=2;$i<=$highestRow;$i++){
        $a = $objPHPExcel->getActiveSheet()->getCell("F".$i)->getValue();//获取订购价格
        $b = $objPHPExcel->getActiveSheet()->getCell("I".$i)->getValue();//product_id
        $j = $shop_id;//商铺ID
        $e = $objPHPExcel->getActiveSheet()->getCell("E".$i)->getValue();//订购方式
